...
def load_ticks(path: Path) -> dict[str, list[dict]]:
    from scripts.analysis.tick_loader import load_ticks as _load
    return _load(path)
...
def main():
    ...
    from scripts.analysis.tick_loader import find_ticks_path
    tick_path = find_ticks_path(run_dir, asset)
    if tick_path is None:
        sys.exit(f"ERROR: ticks file not found in {run_dir}\n"
                 f"  Expected ticks_{asset}.parquet (or .csv) — re-run the dry bot to collect data.")
